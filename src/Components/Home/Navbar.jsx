...
const Navbar = () => {
  ...
  const navLinks = (
    <div className="space-x-12 font-medium text-lg flex flex-col md:flex-row items-center ">
      <NavLink
        className={({ isActive }) =>
          isActive
            ? 'bg-black p-2 border text-white rounded-lg  ml-12'
            : 'ml-12'
        }
        to={'/'}
      >
      ...
      <NavLink
        className={({ isActive }) =>
          isActive ? 'bg-black p-2  text-white rounded-lg' : ' '
        }
        to={'/statistics'}
      >
        Statistics
  ...
  return (
    <div
      className={`navbar sm:px-3 xl:px-28 ${
        pathname === '/' ? 'bg-prime sm:text-white' : 'bg-white text-black'
      }`}
    >
